...
export default function Edit({ skill }) {
    ...
    return (
        <AdminLayout>
            ...
            <div className="mb-6">
                <Link href={route('admin.skills.index')} className="flex items-center text-text-secondary hover:text-text-primary mb-4">
                    <ArrowLeft className="w-4 h-4 mr-2" />
                    Back to Skills
                </Link>
                <h1 className="text-2xl font-bold text-text-primary">Edit Skill: {skill.name}</h1>
            </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-8 max-w-2xl">
                <div className="bg-surface-base border border-border-subtle rounded-lg p-6 space-y-6">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            <label className="block text-sm font-medium text-text-secondary mb-1">Level (0-100)</label>
                            <input
                                type="number"
                                min="0"
                                max="100"
                                ...
                                className="w-full bg-surface-elevated border border-border-subtle rounded-lg px-4 py-2 text-text-primary focus:outline-none focus:border-accent-primary"
                            />
                            {errors.level && <div className="text-semantic-error text-sm mt-1">{errors.level}</div>}
                        </div>
                        <div>
                            ...
                            <input
                                ...
                                onChange={e => setData('years_of_experience', parseInt(e.target.value))}
                                className="w-full bg-surface-elevated border border-border-subtle rounded-lg px-4 py-2 text-text-primary focus:outline-none focus:border-accent-primary"
                            />
                            {errors.years_of_experience && <div className="text-semantic-error text-sm mt-1">{errors.years_of_experience}</div>}
                        </div>
                    </div>
